...
价格样本默认由 **OpenClaw 侧采集** 后通过 **`POST /api/v1/openclaw/monitoring/{monitor_id}/observations/ingest`** 写入服务端；本技能只做 **读取与联合分析**，不依赖服务端 `run-once` 抓网页（默认关闭服务端抓取）。

- `GET /api/v1/openclaw/monitoring/{monitor_id}/summary?window_days={n}`（需 `X-Api-Key`）
- `GET /api/v1/public/monitoring/{monitor_id}/observations?limit={n}`（无需 Key，细粒度拐点）
- `GET /api/v1/public/monitoring/{monitor_id}/timeseries?window_days={n}`（无需 Key，时序）
